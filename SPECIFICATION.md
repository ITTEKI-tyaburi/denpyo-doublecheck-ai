...
## 検算の独立性（合計欄に合わせる推定の全面禁止）

- 合計欄（stated_total）は **比較のためだけ** に使用します。行明細（数量・単価）の決定には一切使用しません。
- 次の順序を常に厳守します。
  1) 時間計算により数量を決定します（数量欄は参照しません）。
  2) 単価は D/S 前置き・固定単価辞書・OCR補正で決定します。
  3) 各行の小計を「数量×単価」で計算します。
  4) 行小計の合計（計算合計）を算出します。
  5) **この時点で初めて** 画像の合計欄（stated_total）を読み取り、計算合計と比較します。
- 以下を **禁止** します。
  - 合計欄に一致させるために数量・単価を変更する行為
  - 行の追加・削除による帳尻合わせ
  - 四捨五入や端数調整による一致化
  - 金額欄（各行の金額）から数量や単価を逆算して充当する行為
- S の 700/1100 のように **同点で複数候補が残る場合** に限り、最後のタイブレークとして合計欄を参照して片方を選ぶことを許容します（それ以外の条件がすべて同点のときのみ）。それでも一意に定まらない場合は一致を目指さず、「合計金額に確認が必要です」とし、備考に候補案を明示します。
- 「推定単価（制約充足）」は **合計欄を目的関数にしてはいけません**。候補集合と数量のみで自立的に決定し、合計欄は最終の同点タイブレークに限定します。
- 出力判定は次の基準に従います。
  - 一致：合計金額は正確です（計算結果と記載合計が一致しました）。
  - 不一致：合計金額に誤りの可能性があります（調整は行いません）。
  - 確認：合計金額に確認が必要です（複数案や読取不確実が残っています）。
- 自己診断ルール：もし合計欄を根拠に数量・単価・行を変更した場合は仕様違反です。AIはその挙動を行わず、必ず独立計算の結果を優先します。
...
- 本推定は候補単価集合と数量のみで決定します。合計欄（stated_total）を目的関数にしてはいけません。
- 合計欄の利用は、S=700/1100 等の **完全同点** が残った場合の最終タイブレークに限ります。一意に決まらなければ一致させず、備考で候補を提示します。
