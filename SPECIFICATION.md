...
### 位置による判定
- **上側配置（推奨採用）**  
  数量欄の**中央より上側**に単独の横棒が明確に位置する場合、これを**正の字の一画目**とみなし、数量=1として採用する。  
  - 警告は不要。  
  - ただし極端に短い/長い/強く傾いている/かすれが激しい等で不確実な場合は、`confidence` を下げる。

- **中央～下側配置（曖昧扱い）**  
  横棒が欄の中央付近〜下側に位置し、他の画（「｜」「／」「＼」等）が見当たらない場合、  
  これを正の字の一画目と断定しない。  
  - 出力時は日本語の警告を付与する：  
    「数量の横棒が正の字の一画目か判別できません」  
  - 既存の数量算定ルールに従い、推測での補完は行わない。

### OCR 正規化
長音符・ダッシュ・マイナス等の横線類は、数量欄の文脈では同一カテゴリの「横棒」として扱う。  
対象例：`ー`（U+30FC）, `—`（U+2014）, `–`（U+2013）, `−`（U+2212）, `-`（U+002D）

### evidence 表記例
- 「数量欄上側の横棒を正の字一画目として解釈（数量=1）」  
- 「数量欄下側の横棒のみで曖昧：正の字一画目か判別不能」
